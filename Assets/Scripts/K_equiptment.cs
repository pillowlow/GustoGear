...
public abstract class K_Equipment : MonoBehaviour
{
    ...
    public void SpawnOutcome()
    {
        ...
        if (toSpawn != null)
        {
            ...
            //interaction label 
            InteractableObjectLabel interactLabel = outcome.GetComponentInChildren<InteractableObjectLabel>();
            if (interactLabel != null)
            {
                interactLabel.playerHead = _playerHead;  // Assigning the _playerHead to playerHead
            }
            else
            {
                Debug.LogError("Spawned object does not have an InteractableObjectLabel component.");
            }
